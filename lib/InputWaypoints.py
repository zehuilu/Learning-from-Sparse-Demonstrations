...
class InputWaypoints(object):
    ...
    def run(self, QuadInitialCondition: QuadStates, QuadDesiredStates: QuadStates, ObsList: list):
        ...
        # plot start and goal
        self.ax_top_down.scatter(QuadInitialCondition.position[0], QuadInitialCondition.position[1], color='green')
        self.ax_top_down.scatter(QuadDesiredStates.position[0], QuadDesiredStates.position[1], color='violet')
        ...
        self.ax_top_down.legend(handles, labels, loc="upper left", framealpha=1)
        ...
        # plot waypoints
        for i in range(len(waypoints_top_down)):
            # print("Waypoint XOY" + str(i+1) + ": x = " + str(waypoints_top_down[i][0]) + ", y = " + str(waypoints_top_down[i][1]))
            self.ax_top_down.scatter(waypoints_top_down[i][0], waypoints_top_down[i][1], color="blue")
        ...
        # plot start and goal
        self.ax_right_left.scatter(QuadInitialCondition.position[0], QuadInitialCondition.position[2], color='green')
        self.ax_right_left.scatter(QuadDesiredStates.position[0], QuadDesiredStates.position[2], color='violet')
        ...
        # plot waypoints
        for i in range(len(waypoints_top_down)):
            self.ax_right_left.scatter(waypoints_top_down[i][0], 0, color="blue")
        ...
        handles = [f(marker_list[i], colors[i]) for i in range(len(labels))]
        # add legend about path
        handles.append(plt.plot([], [], color="black", linestyle='dashed', linewidth=2)[0])
        handles.append(patches.Patch(color="red", alpha=0.75))
        labels.extend(["ground", "Obstacles"])
        self.ax_right_left.legend(handles, labels, loc="upper left", framealpha=1)
        ...
        for i in range(len(waypoints_right_left)):
        ...
        # plot waypoints
        self.ax_3d.plot3D(waypoints_3d_plot[0], waypoints_3d_plot[1], waypoints_3d_plot[2], c="C0")
        for i in range(len(waypoints_output)):
            self.ax_3d.scatter(waypoints_output[i][0], waypoints_output[i][1], waypoints_output[i][2], color="blue")
        ...
        handles.append(patches.Patch(color="red", alpha=0.75))
        labels.append("Obstacles")
        self.ax_3d.legend(handles, labels, loc="upper left", framealpha=1)
